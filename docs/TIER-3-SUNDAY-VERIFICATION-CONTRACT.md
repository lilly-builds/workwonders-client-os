...
3. **Handler:** `/system-health-check` loads one saved copy and its Test Library, runs every test, writes one Health Report on pass, or starts one deep check on failure.
