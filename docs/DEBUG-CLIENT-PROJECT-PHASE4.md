...
- Test Library items require `test_id`, `purpose`, `setup`, `steps`, `expected_result`, `check_method`, `refresh_date`, `scope`, and `owner`. Every item is run; no sampling or reduced coverage is allowed.
...
- `createIdempotencyStore(file)` is the persistence contract. A future real adapter must provide the same `claim(kind, cardId, action)` behavior and retain claims durably.
